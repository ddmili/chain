...
func (p *proofOfWork) run() (int64, []byte, error) {
	...
	go func(t *time.Ticker) {
		for {
			<-t.C
			times += 5
			log.Info("正在挖矿,挖矿区块高度为%d,已经运行%ds,nonce值:%d,当前hash:%x", p.Height, times, nonce, hashByte)
		}
	}(ticker1)
	for nonce < maxInt {
		//检测网络上其他节点是否已经挖出了区块
	...
	//结束计数器
	ticker1.Stop()
	log.Info("本节点已成功挖到区块!!!,高度为:%d,nonce值为:%d,区块hash为: %x", p.Height, nonce, hashByte)
